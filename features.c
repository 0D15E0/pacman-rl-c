...
int get_complex_state_id(void) {
  ...
  if (total_dots > 0) {
    ...
    if (score_ratio > 0.75f)
      score_level = 3;
    else if (score_ratio > 0.50f)
      score_level = 2;
    else if (score_ratio > 0.25f)
      score_level = 1;
  }
  ...
  for (int i = 0; i < 4; i++) {
    ...
    // Clamp values to be safe, as integer division can result in an index equal
    // to GRID_DIVISIONS
    if (grid_x >= GRID_DIVISIONS)
      grid_x = GRID_DIVISIONS - 1;
    if (grid_y >= GRID_DIVISIONS)
      grid_y = GRID_DIVISIONS - 1;
  ...
  const int GHOST_POS_BASE = 9;
  const int VALID_MOVES_BASE = 16;
  const int FRIGHTENED_BASE = 16;

  long idx = score_level; // 4 values
  // Combine ghost positions into the state ID
  for (int i = 0; i < 4; i++) {
    idx = (idx * GHOST_POS_BASE) + ghost_ids[i]; // 9 values per ghost
  }
  idx = (idx * VALID_MOVES_BASE) + valid_mask;
  idx = (idx * FRIGHTENED_BASE) + frightened_mask;
  ...
  if (idx >= COMPLEX_STATE_SPACE_SIZE) {
    ...
    // state space size is calculated correctly. It prevents out-of-bounds
    // access to the Q-Table in case of a logic error in the state calculation.
    fprintf(stderr,
            "Warning: State ID %ld exceeds space size. Capping to %d.\n", idx,
            COMPLEX_STATE_SPACE_SIZE - 1);
