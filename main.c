...
int main(void) {
  ...
  // To change behavior, plug in different functions from features.h and
  // rewards.h.
  ml_agent_init(get_complex_state_id, get_standard_reward,
                COMPLEX_STATE_SPACE_SIZE, NUM_ACTIONS);
  ...
  while (!quit) {
    ...
    if (!is_wall(next_x_desired, next_y_desired)) {
      ...
      action = !is_wall(next_x_straight, next_y_straight) ? pacman.dir
                                                          : desired_action;
    }
    ...
    if (is_wall(next_x, next_y)) {
      ...
      if (map[pacman.y][pacman.x] ==
          TILE_DOT) { // Pacman eats a dot at the new location
        map[pacman.y][pacman.x] = TILE_EMPTY; // Mark as eaten
        current_score += SCORE_DOT;
        pills_eaten++;
      } else if (map[pacman.y][pacman.x] ==
                 TILE_BIG_PILL) {             // Pacman eats a big pill
        map[pacman.y][pacman.x] = TILE_EMPTY; // Mark as eaten
        current_score += SCORE_BIG_PILL;      // Extra points
        pills_eaten++;
        current_mode = MODE_FRIGHTENED;
        mode_timer = 0;
        for (int i = 0; i < 4; i++) {
          if (!ghosts[i].is_eaten)
            ghosts[i].is_frightened = true;
        }
      }
    }
    ...
    // --- COLLISION CHECK 1 (After Pacman Moves) ---
    check_ghost_collisions(&episode_done);

    if (!episode_done) {
      move_ghosts_deterministic(); // Move ghosts

      // --- COLLISION CHECK 2 (After Ghosts Move) ---
      check_ghost_collisions(&episode_done);
    }
    ...
    if (episode_done) {
      after_state.caught_by_ghost = true;
    }

    // Check for win condition (all dots eaten)
    if (pills_eaten >= MAX_SCORE) {
    ...
    if (episode_done) {
      ...
      if (manual_override) {
        desired_action =
            pacman.dir; // Reset desired action to default on new episode
      }
